    ...
    public abstract class BakurBlock : MyGameLogicComponent {
        ...
        */
        ...
        public void AddAngularAcceleration(Vector3D acceleration) {
            IMyCubeGrid grid = block.CubeGrid;
            double pseudoInertiaTensor = grid.Physics.Mass * grid.WorldAABB.Size.Length();

            //MatrixD worldRot = grid.PositionComp.WorldMatrix.GetOrientation();
            //Vector3D worldAABB = Vector3D.Transform(grid.WorldAABB.Size, worldRot);

            AddTorque(acceleration * pseudoInertiaTensor);
        }

        public void AddAngularAcceleration(Vector3D acceleration, Vector3D point) {
            IMyCubeGrid grid = block.CubeGrid;
            double pseudoInertiaTensor = grid.Physics.Mass * grid.WorldAABB.Size.Length();
            AddForce(acceleration * pseudoInertiaTensor, point);
        }
        ...
        public void AddTorque(Vector3D torque) {
            ...
            IMyCubeGrid grid = block.CubeGrid;
            MatrixD invWorldRot = grid.PositionComp.WorldMatrixInvScaled.GetOrientation();
            Vector3D localTorque = Vector3D.Transform(torque * BakurMathHelper.Deg2Rad, invWorldRot);
