...
namespace BakurRepulsorCorp {
    ...
    public class AngularInertialCompensator : BakurBlockEquipment {

        public AngularInertialCompensator(BakurBlock block) : base(block) {
        }
        ...
        public double defaultDumpener = 0.6f;
        ...
        public Vector3D GetDesiredAngularAcceleration(double physicsDeltaTime) {
            ...
            Vector3D localAngularAcceleration = Vector3D.Transform(-(Vector3D)grid.Physics.AngularVelocity / physicsDeltaTime * 0.03f * BakurMathHelper.Rad2Deg * dumpener, invertedOrientation);
        ...
        IMyGyro[] GetGyroscopes() {
            ...
            block.CubeGrid.GetBlocks(blocks, (IMySlimBlock block) => {
                return block.FatBlock is IMyGyro;
            });
            List<IMyGyro> thrusters = new List<IMyGyro>();
            foreach (IMySlimBlock block in blocks) {
                thrusters.Add(block.FatBlock as IMyGyro);
            }
